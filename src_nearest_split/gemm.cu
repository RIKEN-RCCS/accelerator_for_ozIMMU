...
void split_AB_int8_nearest(
    ...
    const std::size_t n,
    const std::size_t k,
    const double *const a_ptr, double *const a_f64_ptr, const std::size_t lda,
    double *const sft_a,
    std::int8_t *const working_a_ptr, const std::uint32_t ld_int8_a,
    const double *const b_ptr, double *const b_f64_ptr, const std::size_t ldb,
    double *const sft_b,
    std::int8_t *const working_b_ptr, const std::uint32_t ld_int8_b,
    std::int8_t num_split,
    std::int8_t bitsA,
    std::int8_t bitsB) {
...
// This function is added
//=====
__global__ void accumulate_in_f64_kernel_3(
    const std::size_t m,
    ...
    const auto mi = tid % m;
    const auto ni = tid / m;
    f64_ptr[tid] += static_cast<double>(i32_ptr[tid]) * sft_a[mi] * sft_b[ni];
}

//=====
// This function is added
//=====
void accumulate_in_f64_3(
    const std::size_t m,
    double *const f64_ptr,
    const std::int32_t *i32_ptr,
    const std::size_t length,
    ...
    accumulate_in_f64_kernel_3<<<(length + block_size - 1) / block_size, block_size, 0, cuda_stream>>>(
        m,
        f64_ptr,
        i32_ptr,
        length,
...
void axby_2(
    ...
    cudaStream_t cuda_stream) {
    constexpr std::size_t block_size = 256;
    axby_kernel_2<<<(m * n + block_size - 1) / block_size, block_size, 0, cuda_stream>>>(
        m, n,
        a,
        x_ptr,
        b,
...
void matmul_core(
    ...
    case mtk::ozimmu::detail::int8tc: {
        ...
        while (rem > 0) {
            ...
            CUTF_CHECK_ERROR_M(
                cublasGemmEx_org(handle->cublas_handle, op_A_r, op_B_r, m, nn, k,
                                 &alpha_i, a_ptr_r, CUDA_R_8I, lda_r, b_ptr_r + offset * ldb_r,
                                 CUDA_R_8I, ldb_r, &beta_i, c_ptr_r + offset * m * 4, CUDA_R_32I, m,
...
int gemm_int8<double>(mtk::ozimmu::handle_t handle,
                      ...
                      const mtk::ozimmu::compute_mode_t compute_mode) {
    const std::int8_t num_split = mtk::ozimmu::detail::get_split_config(compute_mode).matrix_A_split_types.size() - 1;
    const std::int8_t bitsA     = std::min<std::int8_t>(7, int8_t(std::ceil((31 - std::log2(float(k))) * 0.5)));
    const std::int8_t bitsB     = std::min<std::int8_t>(7, int8_t(std::floor((31 - std::log2(float(k))) * 0.5)));

    double *const c_f64_ptr        = reinterpret_cast<double *>(handle->working_memory_ptr);
    double *const a_f64_ptr        = c_f64_ptr + m * n;
    double *const b_f64_ptr        = a_f64_ptr + m * k;
    double *const sft_a            = b_f64_ptr + k * n;
    double *const sft_b            = sft_a + m * num_split;
    std::int32_t *const c_i32_ptr  = reinterpret_cast<int32_t *>(sft_b + n * num_split);
    ...
    for (const auto &gemm_pair_config : gemm_pair_config_list) {
        ...
        handle->profiler.start_timer_sync("accumulate_in_f64_3");
        accumulate_in_f64_3(
            m,
            c_f64_ptr,
            c_i32_ptr,
            m * n,
            sft_a + (gemm_pair_config.A_id - 1) * m,
            sft_b + (gemm_pair_config.B_id - 1) * n,
            handle->cuda_stream);
        handle->profiler.stop_timer_sync("accumulate_in_f64_3");
    }
