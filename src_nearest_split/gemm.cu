...
int gemm_int8<double>(mtk::ozimmu::handle_t handle,
  ...
  const std::int8_t bitsA = std::min<std::int8_t>(7, std::ceil((31 - std::log2(k)) * 0.5));
  const std::int8_t bitsB = std::min<std::int8_t>(7, std::floor((31 - std::log2(k)) * 0.5));
