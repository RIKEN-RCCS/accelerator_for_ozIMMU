...
std::size_t mtk::ozimmu::reallocate_working_memory(
  ...
  for (const auto gemm : gemm_list) {
    ...
    const auto working_memory_A_fp64 =
        m * k * mtk::ozimmu::get_data_size_in_byte(fp64) *
        (element_kind == mtk::ozimmu::real ? 1 : 0);
    const auto working_memory_B_fp64 =
        k * n * mtk::ozimmu::get_data_size_in_byte(fp64) *
        (element_kind == mtk::ozimmu::real ? 1 : 0);
    std::size_t etc = 0;
