...
__global__ void extract_int8_kernel(
    ...
    for (unsigned i = threadIdx.x; i < n; i += blockDim.x) {
        unsigned idx                  = (col_major ? (i * ld + row_index) : (i + row_index * ld));
        in_ptr_tmp[row_index * n + i] = in_ptr[idx];
    }

    const auto N = m * ldo;
    double t     = scalbn(1.5, 53 - bits);
    double s     = scalbn(1.0, bits - 1);
    for (unsigned id = 0; id < num_split; ++id) {
        const double amax = find_amax(in_ptr_tmp + (row_index * n), n, 1, smem);
        // double NPT = get_npt(amax);
        // const double tt = t * NPT;
        // const double ss = s / NPT;
        const short log2_npt = get_exp_npt(amax);
        const double tt      = scalbn(t, log2_npt);
        const double ss      = scalbn(s, -log2_npt);
        unsigned i;
        for (i = threadIdx.x; i < n; i += blockDim.x) {
            double &a  = in_ptr_tmp[i + row_index * n];
            double a_s = (a + tt) - tt;
            a -= a_s;
            *(out_ptr + row_index * ldo + i + N * id) = static_cast<std::int8_t>(a_s * ss);
        }
        // Fill the padding elements with zeros
        for (; i < ldo; i += blockDim.x) {
            *(out_ptr + row_index * ldo + i + N * id) = 0;
        }
        if (threadIdx.x == 0) {
            sft[blockIdx.x + id * m] = 1.0 / ss;
            // sft[blockIdx.x + id * m] = scalbn(1.0, 1 - bits + log2_npt);
        }
    }
}
