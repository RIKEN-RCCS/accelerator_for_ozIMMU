...
namespace {
  ...
  __device__ double get_npt(double x) {
	...
	__global__ void extract_int8_kernel(
		...
		const auto N = m * ldo;
		double t = ldexp(1.5, 53 - bits);
		double s = ldexp(1.0, bits - 1);
      ...
      // const double ss = s / NPT;
			...
			const double tt = ldexp(t,log2_npt);
			const double ss = ldexp(s,-log2_npt);
      unsigned i;
        ...
        *(out_ptr + row_index * ldo + i + N * id) = static_cast<std::int8_t>(a_s*ss);
        ...
        sft[blockIdx.x + id * m] = 1.0/ss;
				// sft[blockIdx.x + id * m] = ldexp(1.0, 1 - bits + log2_npt);
			}
		}
	}
