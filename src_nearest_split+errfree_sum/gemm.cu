...
void accumulate_in_f64_2(
    ...
    accumulate_in_f64_kernel_2<<<(length + block_size - 1) / block_size, block_size, 0, cuda_stream>>>(
        m,
        f64_ptr,
        i32_ptr,
        length,
    ...
    if (lim_accum == 0) {
        ...
        for (const auto &gemm_pair_config : gemm_pair_config_list) {
            ...
            accumulate_in_f64_2(
                ...
                a_max_exp_ptr,
                b_max_exp_ptr,
                scalbn(1.0, -bits_per_int8 * (gemm_pair_config.A_id + gemm_pair_config.B_id - 2)),
                handle->cuda_stream);
